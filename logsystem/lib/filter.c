...
MODULE("@(#)TradeXpress $Id: filter.c 55487 2020-05-06 08:56:27Z jlebiannic $")
/*========================================================================
  ...
  4.01 12.07.02/CD	Clear separator in dao_logfilter_clear
                    Allow to sort in reverse INDEX order 
			        Change in dao_logfilter_setorder for the -d and -a option of logview
...
static void dao_topieces(char *namevalue, char **name, int *cmp, char **value)
{
	char *cp = namevalue;
	char *name_end;
...
static int dao_atoorder(char *a)
{
	switch (*a)
	{
		case 'a':
...
static LogFilter * dao_alloc_filter()
{
	LogFilter *lf;

	lf = dao_log_malloc(sizeof(*lf));
...
int dao_logfilter_type(char *cmp)
{
	unsigned int i;
...
void dao_logfilter_insert(LogFilter **lfp, char *name, int cmp, char *value)
{
	FilterRecord *f;
	LogFilter *lf;
	char *filter_escape;
	...
	if (lf->filter_count >= lf->filter_max)
	{
		...
		lf->filters = dao_log_realloc(lf->filters, n);
		memset(lf->filters + omax, 0, n - omax * sizeof(*lf->filters));
	}
	f = &lf->filters[lf->filter_count++];

	f->name = dao_log_strdup(name);
	f->op = cmp;
	f->value = dao_log_strdup(value);
}

int dao_logfilter_printables(LogFilter *lf, char *name, char **cmp, char **value)
{
	FilterRecord *f;
...
char * dao_logfilter_getseparator(LogFilter *lf)
{
	/* 4.00/CD */
	if (!lf)
		return (NULL);
...
char * dao_logfilter_getkey(LogFilter *lf)
{
	if (!lf)
		return (NULL);
	return (lf->sort_key);
...
int dao_logfilter_getorder(LogFilter *lf)
{
	if (!lf)
		return (0);
...
int dao_logfilter_getoffset(LogFilter *lf)
{
	if (!lf)
		return (0);
	return (lf->offset);
...
int dao_logfilter_getmaxcount(LogFilter *lf)
{
	if (!lf)
		return (0);
	return (lf->max_count);
...
char ** dao_logfilter_textual(LogFilter *lf)
{
	FilterRecord *f;
	char **v;
	int i;
	...
	v = dao_log_malloc((lf->filter_count + 1) * sizeof(*v));
	i = 0;

	for (f = lf->filters; f < &lf->filters[lf->filter_count]; ++f)
	{
		v[i++] = dao_log_str3dup(f->name, operator_strings[f->op], f->value);
	}
	v[i] = NULL;
...
/* Return textual representation of filter (just the filter)
 * Use next function to free up space. */
char ** dao_logfilter_textualparam(LogFilter *lf)
{
	FilterRecord *f;
	char **v;
	int i;
	...
	v = dao_log_malloc((lf->filter_count + 6) * sizeof(*v));
	i = 0;
	...
	if (lf->filter_count)
	{
		for (f = lf->filters; f < &lf->filters[lf->filter_count]; ++f)
		{
			v[i] = dao_log_malloc(1 /* p|f|'' */ + strlen(f->name) + strlen(operator_strings[f->op]) + strlen(f->value) + 1);
			strcpy(v[i], "f");
			strcat(v[i], f->name);
	...
	if (lf->sort_key)
		v[i++] = dao_log_str3dup("SORTKEY", "=", lf->sort_key);

	if (lf->sort_order)
	{
		/* XXX use MAX_DIGIT or better alloc */
		v[i] = dao_log_malloc(sizeof("SORTORDER=") + 2);
	...
	{
		/* XXX use MAX_DIGIT or better alloc */
		v[i] = dao_log_malloc(sizeof("MAXCOUNT=") + 10);
	...
	{
		/* XXX use MAX_DIGIT or better alloc */
		v[i] = dao_log_malloc(sizeof("OFFSET=") + 10);
		sprintf(v[i], "OFFSET=%d", lf->offset);
		i++;
	}

	if (lf->separator)
		v[i++] = dao_log_str3dup("SEPARATOR", "=", lf->separator);
...
void dao_logfilter_textual_free(char **v)
{
	int i;
...
void dao_logfilter_setseparator(LogFilter **lfp, char *sep)
{
	/* 4.00/CD */
	if (!*lfp)
		*lfp = dao_alloc_filter();
	if (sep)
		sep = dao_log_strdup(sep); /* dup before free, in case they are same... */
	if ((*lfp)->separator)
		free((*lfp)->separator);
...
void dao_logfilter_setkey(LogFilter **lfp, char *key)
{
	if (!*lfp)
		*lfp = dao_alloc_filter();
	if (key)
		key = dao_log_strdup(key); /* dup before free, in case they are same... */
	if ((*lfp)->sort_key)
		free((*lfp)->sort_key);
...
void dao_logfilter_setoffset(LogFilter **lfp, int offset)
{
	if (!*lfp)
		*lfp = dao_alloc_filter();
	if (offset > 0)
...
void dao_logfilter_setorder(LogFilter **lfp, int order)
{
...
void dao_logfilter_setmaxcount(LogFilter **lfp, int maxcount)
{
	if (!*lfp)
		*lfp = dao_alloc_filter();
...
void dao_logfilter_add(LogFilter **lfp, char *namevalue)
{
	char *name, *value;
	int cmp;

	dao_topieces(namevalue, &name, &cmp, &value);

	if (name && value)
		dao_logfilter_insert(lfp, name, cmp, value);
}

void dao_logfilter_setenv(LogFilter **lfp, int cmp, char *value)
{
	if (!*lfp)
		*lfp = dao_alloc_filter();
	
	if (value)
		value = dao_log_strdup(value); /* dup before free, in case they are same... */
	if ((*lfp)->env_val)
		free((*lfp)->env_val);
...
int dao_logfilter_read(LogFilter **lfp, FILE *fp)
{
	char buf[1024];
	char *name, *value, *cp;
	int cmp;
	...
	while (fp && tr_mbFgets(buf, sizeof(buf), fp)) {
		...
		dao_topieces(buf, &name, &cmp, &value);

		if (!strcmp(name, "SORTKEY"))
		{
			dao_logfilter_setkey(lfp, value);
			continue;
		}
		if (!strcmp(name, "SORTORDER"))
		{
			dao_logfilter_setorder(lfp, dao_atoorder(value));
			continue;
		}
		if (!strcmp(name, "MAXCOUNT"))
		{
			dao_logfilter_setmaxcount(lfp, atoi(value));
			continue;
		}
		if (!strcmp(name, "OFFSET"))
        {
			dao_logfilter_setoffset(lfp, atoi(value));
			continue;
		}
		/* 4.00/CD */
		if (!strcmp(name, "SEPARATOR"))
		{
			dao_logfilter_setseparator(lfp, value);
			continue;
		}
		...
		if(!strcmp(name,"ENV"))
		{
			dao_logfilter_setenv(lfp, cmp, value);
			continue;
		}
		/*END TX-2600*/
		dao_logfilter_insert(lfp, name, cmp, value);
...
void dao_logfilter_addparam(LogFilter **lfp, PrintFormat **pfp, char *namevalue)
{
	char *name, *value;
	int cmp;
	...
	switch (*name)
	{
		default:
			dao_logfilter_insert(lfp, name, cmp, value);
			dao_printformat_insert(pfp, name, NULL);
			break;
		case 'f': dao_logfilter_insert(lfp, name + 1, cmp, value); break;
		case 'p': dao_printformat_insert(pfp, name + 1, NULL); break;
	}
}

int dao_logfilter_printformat_read(LogFilter **lfp, PrintFormat **pfp, FILE *fp)
{
	char buf[1024];
	char *cp;
	...
	while (fp && tr_mbFgets(buf, sizeof(buf), fp))
	{
		...
		dao_logfilter_addparam(lfp, pfp, buf);
	}
	return (0);
}

int dao_logfilter_write(LogFilter *lf, FILE *fp)
{
	FilterRecord *f;
...
int dao_logfilter_printformat_write(LogFilter *lf, PrintFormat *pf, FILE *fp)
{
	FilterRecord *f;
	FormatRecord *r;
...
void dao_logfilter_clear(LogFilter *lf)
{
	FilterRecord *f;
...
void dao_logfilter_free(LogFilter *lf)
{
	if (!lf)
		return;

	dao_logfilter_clear(lf);
...
static int dao_wildcompare(register char *s/* string */, register char *w/* pattern */)
{
	while (*w)
	{
		switch (*w)
...
static union FilterPrimary * dao_new_primary(FilterRecord *f)
{
	f->primaries = dao_log_realloc(f->primaries, (f->n_primaries + 1) * sizeof(*f->primaries));
...
int dao_logsys_compilefilter(LogSystem *ls, LogFilter *lf)
{
	...
	for (f = lf->filters; f < &lf->filters[lf->filter_count]; ++f)
	{
		...
		f->field = dao_logsys_getfield(ls, f->name);
		if (!f->field)
		{
			++nunk;
			continue;
...
static int dao_logfilter_textsort_forward    AL{return(strcmp((char*) A, (char *)       B));}
static int dao_logfilter_textsort_reverse    AL{return(strcmp((char*) B, (char *)       A));}
static int dao_logfilter_timesort_forward    AL{return(*(TimeStamp*)  A - *(TimeStamp*) B);}
static int dao_logfilter_timesort_reverse    AL{return(*(TimeStamp*)  B - *(TimeStamp*) A);}
static int dao_logfilter_numbersort_forward  AL{return(*(Number*)     A - *(Number*)    B);}
static int dao_logfilter_numbersort_reverse  AL{return(*(Number*)     B - *(Number*)    A);}
static int dao_logfilter_integersort_forward AL{return(*(Integer*)    A - *(Integer*)   B);}
static int dao_logfilter_integersort_reverse AL{return(*(Integer*)    B - *(Integer*)   A);}
...
int dao_logsys_sortentries(LogEntry **entries, int num_entries, LogFilter *lf)
{
	LogSystem *log;
	LogField *keyfield;
...

	{
		...
		keyfield = dao_logsys_getfield(log, lf->sort_key);
		if (!keyfield)
			goto skip_sort;
	...
	switch (keyfield->type)
	{
		...
		case FIELD_TEXT:
			if (lf->sort_order > 0)
				sorter = dao_logfilter_textsort_forward;
			else
				sorter = dao_logfilter_textsort_reverse;
			break;
		case FIELD_TIME:
			if (lf->sort_order > 0)
				sorter = dao_logfilter_timesort_forward;
			else
				sorter = dao_logfilter_timesort_reverse;
			break;
		case FIELD_INDEX:
		case FIELD_INTEGER:
			if (lf->sort_order > 0)
				sorter = dao_logfilter_integersort_forward;
			else
				sorter = dao_logfilter_integersort_reverse;
			break;
		case FIELD_NUMBER:
			if (lf->sort_order > 0)
				sorter = dao_logfilter_numbersort_forward;
			else
				sorter = dao_logfilter_numbersort_reverse;
			break;
	}
